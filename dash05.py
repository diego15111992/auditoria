...
st.title("Dasboard Devoluções MRZ Group - Ano de 2025")
...
top_5_motivos = motivos_df.head(5)[["MOTIVO", "PERCENTUAL"]]
...
def gerar_ranking_df(df, coluna, titulo):
    if coluna in df.columns:
        ...
        rnk["VALOR"] = rnk["VALOR"].apply(lambda x: locale.currency(x, grouping=True))
        return titulo, rnk
    return None, None
...
top_10_motivos_valor["VALOR"] = top_10_motivos_valor["VALOR"].apply(lambda x: locale.currency(x, grouping=True))
